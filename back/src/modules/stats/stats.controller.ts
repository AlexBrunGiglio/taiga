...
export class StatsController extends BaseController {
  ...
  @ApiDocs({
    summary: 'Create or update stat',
    operationId: 'createOrUpdateStat',
    resStatus: HttpStatus.CREATED,
    resType: GetStatResponse,
  })
  async createOrUpdate(@Body() stat: StatDto): Promise<GetStatResponse> {
    if (!stat) throw new BadRequestException('Invalid Request');
