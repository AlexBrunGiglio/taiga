...
export class ReferentialService extends ApplicationBaseService {
  ...
  async getOneAppType(id: string): Promise<GetAppTypeResponse> {
    ...
    try {
      ...
      if (!appType) throw new AppError('app type not found !');
  ...
  async getTypeValues(
    ...
    try {
      ...
      if (request.alsoDisabled !== 'true')
        response.appType.appValues = response.appType.appValues.filter(
          x => !!x.enabled,
        );
      response.appType.appValues.sort((a, b) => {
        if (a[request.orderby] === b[request.orderby]) return 0;
  ...
  async getMultipleTypeValues(
    ...
    try {
      ...
      if (!appTypeCodes?.length)
        throw new AppError('You must specify a type code !');
      const findOptions = BaseSearchRequest.getDefaultFindOptions<AppType>(
        request,
      );
  ...
  async insertOrUpdateAppType(
    ...
    const response = new GetAppTypeResponse();
    try {
      if (!appTypeDto.code) throw new AppError('app type code must be set !');
      const relations: string[] = [];
      if (includeAppValues) {
        relations.push('appValues');
        if (includeTranslations) relations.push('appValues.translations');
      }
      let appType: AppType = await this.appTypesRepository.findOne({
        where: { code: appTypeDto.code },
        relations: relations,
      });
      if (!appType) appType = new AppType();
      else appTypeDto.id = appType.id;
  ...
  async insertOrUpdateAppValue(
    ...
    try {
      ...
      if (!appValue) appValue = new AppValue();
      else if (appValue.id) appValueDto.id = appValue.id;
  ...
  public async removeAppValues(ids?: string[], codes?: string[]) {
    ...
    try {
      if (!ids?.length && !codes?.length) throw new AppError('Invalid args !');
