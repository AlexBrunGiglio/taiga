import { BadRequestException, Body, Controller, Delete, ForbiddenException, Get, HttpStatus, Param, Patch, Post, Query } from '@nestjs/common';
...
export class UsersController extends BaseController {
    ...
    async getAll(@Query() request: GetUsersRequest): Promise<GetUsersResponse> {
        ...
        if (request.search) {
            if (!findOptions.where)
                findOptions.where = [{}];
            findOptions.where = [{ firstname: Like('%' + request.search + '%') }, { lastname: Like('%' + request.search + '%') }];
        }
    ...
    @Get(':id')
    @UserLogged()
    @ApiDocs({ summary: 'Get user', operationId: 'getUser', resStatus: HttpStatus.OK, resType: GetUserResponse })
    ...
    @ApiDocs({ summary: 'Create or update user', operationId: 'createOrUpdateUser', resStatus: HttpStatus.CREATED, resType: GetUserResponse })
    ...
    @ApiDocs({ summary: 'Archive my account', operationId: 'archiveMyAccount', resStatus: HttpStatus.OK, resType: GenericResponse })
    ...
    @AllowRoles(RolesList.Visitor)
    @ApiDocs({ summary: 'Delete account', operationId: 'deleteAccount', resStatus: HttpStatus.OK, resType: GenericResponse })
