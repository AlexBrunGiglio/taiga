...
> extends ApplicationBaseService {
  ...
  private getLinqRelationsFromArray(
    ...
    for (const relation of relations) {
      ...
      if (relation.child) query = query.thenInclude(relation.child.include);
    }
    return query;
  }
  async findOne(
    ...
    try {
      ...
      if (useNewLinqRepo) {
        ...
        conditions.relations = new List([...conditions.relations, ...relations])
          .Distinct()
          .ToArray();
  ...
  async findAll(
    ...
    try {
      ...
      const useNewLinqRepo = !!(conditions as any).query;
      if (useNewLinqRepo)
        response.filteredResults = await ((conditions as any).query as IQuery<
          X,
          X[],
          X
        >).count();
    ...
    try {
      if (ids && ids.length > 0) {
        for (const id of ids) {
          ...
          if (
            getOneResponse.success &&
            getOneResponse[this.modelOptions.getOneResponseField]
          ) {
            getOneResponse[this.modelOptions.getOneResponseField][
              this.modelOptions.archiveField
            ] = this.modelOptions.archiveFieldValue;
