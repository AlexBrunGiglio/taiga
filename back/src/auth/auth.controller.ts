...
export class AuthController extends BaseController {
    ...
    @Post('login')
    @ApiDocs({ summary: 'Connexion d\'un utilisateur', operationId: 'login', resStatus: HttpStatus.OK, resType: LoginResponse })
    async login(@Body() loginViewModel: LoginViewModel): Promise<LoginResponse> {
        return await this.authService.login(loginViewModel);;
    }

    @Post('register')
    @ApiDocs({ summary: 'Inscription d\'un utilisateur', operationId: 'register', resStatus: HttpStatus.CREATED, resType: LoginResponse })
    ...
    @ApiDocs({ summary: 'Déconnexion d\'un utilisateur', operationId: 'logout', resStatus: HttpStatus.OK, resType: GenericResponse })
    ...
    @ApiDocs({ summary: 'Création d\'un refresh token à partir d\'un token', operationId: 'refreshToken', resStatus: HttpStatus.OK, resType: GenericResponse })
    async refresh(@Param('token') refreshToken: string): Promise<GenericResponse> {
        return await this.authService.refreshToken(refreshToken);
    }

    @Patch('activate-account')
    @ApiDocs({ summary: 'Activation du compte', operationId: 'activateAccount', resStatus: HttpStatus.CREATED, resType: GenericResponse })
